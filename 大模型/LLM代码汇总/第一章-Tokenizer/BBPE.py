...
def bytes_to_unicode():
    """
    返回utf-8字节列表和到unicode字符串的映射。避免映射到bbpe代码所依赖的空白/控制字符。
    可逆的bbpe代码在unicode字符串上工作。如果想避免UNKs，需要在词汇表中使用大量的unicode字符。
    当有一个10B的token数据集时，最终需要大约5K才能获得良好的覆盖。这是你正常情况下的一个显著比例，
...
class BBPETokenizer(nn.Module):
    ...
    def encode(self, text: str) -> list[int]:
        ...
        for token in re.findall(self.pat, text):
            ...
            # 对token执行bbpe合并，根据编码器将结果映射到bbpe索引
