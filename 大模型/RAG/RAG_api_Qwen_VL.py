...
CHAT_MODEL        = "gpt-3.5-turbo"  # 用于 query 改写
...
def rewrite_query(original_query: str) -> str:
    """
    调用 ChatCompletion 将用户原始查询改写为更适合检索的形式。因为用户自然语言冗长含糊 改写后能提升向量检索的召回质量
    """
...
def generate_caption_api(image_path):
    """同前，用 Qwen-VL 生成图片说明。"""
    uri = f"file://{os.path.abspath(image_path)}"  #图片路径转为 file:// URI
...
def generate_qwen_vl_response_api(query, retrieved, max_images=1):
    ...
    system = {"role":"system","content":[{"text":"You are a helpful assistant. Answer ONLY from provided context."}]} #限定只能根据提供的上下文回答
    user_content = []
    # 添加入选图像 从检索结果中取得分最高的图片（最多 max_images=1 张）
    added=0
